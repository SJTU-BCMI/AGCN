...
if __name__ == "__main__":
    ...
    for epoch in range(50):
        for step, ((b_x, b_y),(b_x_t, _)) in enumerate(zip(train_loader,test_loader_d)):
            b_x = b_x.float()
            b_x = b_x
            b_y = b_y
            b_y = b_y.long()
            b_x_t = b_x_t.float()
            b_x_t = b_x_t
            b_y_domain=torch.zeros(b_y.shape[0], dtype=torch.long)
            b_y=b_y[:,0]
            ...
            _, output_domain_t, _ , _ = m(b_x_t)
            loss_domain_t = loss_function(output_domain_t, torch.ones(b_x_t.shape[0], dtype=torch.long))
            loss_adv+=loss_domain_t
            loss+=l_adv*loss_adv
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
